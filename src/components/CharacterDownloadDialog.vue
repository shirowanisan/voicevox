      ...
      <q-page-container>
        <q-page class="main">
          <div class="character-items-container">
            ...
            <p class="q-ma-sm">
              ここでダウンロードした音声はspeaker_infoに格納されます。この音声のフォルダ名を変更すると、音声アップデートの際に過去バージョンを上手く削除できず、起動時にキャラクター重複でエラーになるためご注意ください。
            </p>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const downloadErrorMap = computed(() => {
      const map: { [key: string]: boolean } = {};
      props.downloadInfos.forEach((downloadInfo) => {
        map[downloadInfo.downloadableModel.speaker.speakerUuid] = false;
      });
